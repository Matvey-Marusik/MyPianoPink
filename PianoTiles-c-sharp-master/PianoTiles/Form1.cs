        ...
        public void DrawMap(Graphics g)
        {
            for(int i = 0; i < 8; i++)
            {
                for(int j = 0; j < 4; j++)
                {
                    if(map[i,j] == 0)
                    {
                        g.FillRectangle(new SolidBrush(Color.Pink), cellWidth * j, cellHeight * i, cellWidth, cellHeight);
                    }
                    if(map[i,j] == 1)
                    {
                        g.FillRectangle(new SolidBrush(Color.Fuchsia), cellWidth * j, cellHeight * i, cellWidth, cellHeight);
                    }
                }
            }
            for(int i = 0; i < 8; i++)
            {
                g.DrawLine(new Pen(new SolidBrush(Color.Purple)), 0, i * cellHeight, 4 * cellWidth, i * cellHeight);
            }
            for(int i = 0; i < 4; i++)
            {
                g.DrawLine(new Pen(new SolidBrush(Color.Purple)), i * cellWidth, 0, i * cellWidth, 8 * cellHeight);
            }
        }
